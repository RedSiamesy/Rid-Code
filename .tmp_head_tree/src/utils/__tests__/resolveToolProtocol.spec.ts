...
describe("resolveToolProtocol", () => {
	 ...
	 *
	 * XML tool protocol is deprecated, but still used when a model explicitly
	 * does not support native tools. User preferences and model defaults are ignored.
	 *
	 * Precedence:
	...
	describe("Protocol Selection For New Tasks", () => {
		it("should use native for new tasks by default", () => {
			const settings: ProviderSettings = {
				apiProvider: "anthropic",
			}
		...
		it("should use XML when model does not support native tools", () => {
			const settings: ProviderSettings = {
				apiProvider: "openai",
			}
			const result = resolveToolProtocol(settings, { supportsNativeTools: false })
			expect(result).toBe(TOOL_PROTOCOL.XML)
		})
	})
	...
	describe("Real-world Scenarios", () => {
		...
		it("should use XML when a model explicitly disables native tools", () => {
			const settings: ProviderSettings = {
				apiProvider: "openai",
			}
			const modelInfo: ModelInfo = {
