 ...
 * Also consolidates sequences of response-bearing asks and mcp_server_response messages.
 *
...
export function consolidateCommands(messages: ClineMessage[]): ClineMessage[] {
	...
	for (let i = 0; i < messages.length; i++) {
		...
		// Handle response-bearing asks (MCP/web search/url fetch)
		if (msg.type === "ask" && msg.ask && RESPONSE_EXPECTING_ASKS.has(msg.ask)) {
			...
			while (j < messages.length) {
				...
				if (nextMsg.say === "mcp_server_response") {
					...
					j++
				} else if (nextMsg.type === "ask" && nextMsg.ask && RESPONSE_EXPECTING_ASKS.has(nextMsg.ask)) {
