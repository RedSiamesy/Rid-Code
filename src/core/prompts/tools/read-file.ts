...
export function getReadFileDescription(args: ToolArgs): string {
	const maxConcurrentReads = args.settings?.maxConcurrentFileReads ?? 5
  const maxLinesCount = args.settings?.maxReadFileLine ?? 500
	const isMultipleReadsEnabled = maxConcurrentReads > 1

	return `## read_file
Description: Reads ${isMultipleReadsEnabled ? "one or more files" : "a file"} from the local filesystem. The tool outputs line-numbered content (e.g. "1 | const x = 1") for easy reference when creating diffs or discussing code.${args.partialReadsEnabled ? " Use line ranges to efficiently read specific portions of large files." : ""} Supports text extraction from PDF and DOCX files, but may not handle other binary files properly.

${isMultipleReadsEnabled ? `- You can read a maximum of ${maxConcurrentReads} files in a single tool request. ` : "- Multiple file reads are currently disabled. You can only read one file at a time."}
${args.partialReadsEnabled ? `- By default, it reads up to ${maxLinesCount} lines.` : ""}
...
<args>
  <file>
    <path>src/app.ts</path>
    ${args.partialReadsEnabled ? `<line_range>100-1000</line_range>` : ""}
  </file>
</args>
</read_file>
...
- ${isMultipleReadsEnabled ? `You MUST read all related files and implementations together in a single 'read_file' operation (up to ${maxConcurrentReads} files at once)` : "You MUST read files one at a time, as multiple file reads are currently disabled"}
- You MUST obtain all necessary context before proceeding with changes
${
	args.partialReadsEnabled
		? partialReadsGuide
...
export function getReadFileOpenAIToolDefinition(args: ToolArgs): OpenAIToolDefinition {
	const maxConcurrentReads = args.settings?.maxConcurrentFileReads ?? 5
	const maxLinesCount = args.settings?.maxReadFileLine ?? 500
	const isMultipleReadsEnabled = maxConcurrentReads > 1
	...
	if (args.partialReadsEnabled) {
		if (isMultipleReadsEnabled) {
			...
			properties.start_line = {
				type: "string",
				description: "String of an integer representing the start line (inclusive)",
			}
			properties.end_line = {
				type: "string",
				description: "String of an integer representing the end line (inclusive)",
			}
		}
	}
	...
	if (isMultipleReadsEnabled) {
			...
			function: {
				...
				parameters: {
					...
					properties: {
						args: {
							type: "array",
							description: `Array of files to read (maximum ${maxConcurrentReads} files)`,
							items: {
								type: "object",
								properties,
								required
		...
		return {
			...
			function: {
				name: "read_file",
				description: `Reads a file from the local filesystem. Outputs line-numbered content for easy reference. Supports text extraction from PDF and DOCX files. Multiple file reads are currently disabled. You can only read one file at a time. By default, reads up to ${maxLinesCount} lines.${args.partialReadsEnabled ? " Use line ranges to efficiently read specific portions of large files." : ""}`,
				parameters: {
					type: "object",
					properties,
					required
