...
export class Task extends EventEmitter<TaskEvents> implements TaskLike {
	...
	async ask(
		...
		isProtected?: boolean,
		options?: { forceAutoApprove?: boolean },
		...
		const forceAutoApprove = options?.forceAutoApprove === true

		// Automatically approve if the ask according to the user's settings.
		const provider = this.providerRef.deref()
		const state = provider ? await provider.getState() : undefined
		const approval = forceAutoApprove
	...
	public async summarizeSubTaskContext(): Promise<string> {
		...
		const { summary, error } = await summarizeSubTask(
			this,
			effectiveHistory,
			this.api,
	...
	public async recursivelyMakeClineRequests(
		...
		while (stack.length > 0) {
			...
			let userExecuteCommandResult: string | undefined
			...
			let finalUserContent = [
				...
				...(userExecuteCommandResult !== undefined
					? [
							{
								type: "text" as const,
								text: `User executed a command in the local terminal. Output:\n<user_execute_command_result>\n${userExecuteCommandResult}\n</user_execute_command_result>`,
							},
						]
					: []),
			...
			try {
				...
				try {
					...
					const drainStreamInBackgroundToFindAllUsage = async (apiReqIndex: number) => {
						...
						let bgTps = currentTokens.tps
						let bgLatency = currentTokens.latency
						...
						const captureUsageData = async (
							...
							if (
								...
								if (typeof tokens.tps === "number") {
									tps = tokens.tps
								}
								if (typeof tokens.latency === "number") {
									latency = tokens.latency
								}
						...
						try {
							...
							while (!item.done) {
								...
								if (chunk && chunk.type === "usage") {
									...
									bgLatency += chunk.latency ?? 0
									bgTps = chunk.tps ?? bgTps
								}
							}
