...
import { ApiStream, GroundingSource, ApiStreamToolChunk } from "../../api/transform/stream"
...
import { SYSTEM_PROMPT, OPENAI_TOOLS_LIST } from "../prompts/system"
...
export class Task extends EventEmitter<TaskEvents> implements TaskLike {
	...
	async sayAndCreateMissingParamError(toolName: ToolName, paramName: string, relPath?: string) {
		...
		return formatResponse.toolError(formatResponse.missingToolParameterError(paramName, experiments?.useToolCalling))
	}
	...
	private async initiateTaskLoop(userContent: Anthropic.Messages.ContentBlockParam[]): Promise<void> {
		...
		while (!this.abort) {
			...
			if (didEndLoop) {
				...
				const state = await this.providerRef.deref()?.getState()
				const { experiments } = state ?? {}
				nextUserContent = [{ type: "text", text: formatResponse.noToolsUsed(experiments?.useToolCalling) }]
		...
		while (stack.length > 0) {
			...
			try {
				...
				try {
					...
					while (!item.done) {
						...
						switch (chunk.type) {
							...
							case "tool": {
								this.toolsCalledThisTurn.push(chunk)
								const tool : AssistantMessageContent = {
									type: "tool_use",
								...
								const prevLength = this.assistantMessageContent.length
								this.assistantMessageParser.contentBlocks.push(tool)
				...
				if (assistantMessage.length > 0) {
					...
					await this.addToApiConversationHistory({
						role: "assistant",
							content: [{ type: "text", text: assistantMessage }, ...this.toolsCalledThisTurn.map((tool) => {
								return {
					...
					if (!didToolUse) {
						const state = await this.providerRef.deref()?.getState()
						const { experiments } = state ?? {}
						this.userMessageContent.push({ type: "text", text: formatResponse.noToolsUsed(experiments?.useToolCalling) })
						this.consecutiveMistakeCount++
					}
	...
	private async getSystemTools(): Promise<any> {
		// Check if useToolCalling experiment is enabled
		const state = await this.providerRef.deref()?.getState()
		const { experiments } = state ?? {}

		// If useToolCalling is not enabled, return undefined
		if (!experiments?.useToolCalling) {
			return undefined
		}
		...
		if (mcpEnabled ?? true) {
			...
			// Wait for MCP servers to be connected before generating tools list
