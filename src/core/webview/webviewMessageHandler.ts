...
export const webviewMessageHandler = async (
	...
	switch (message.type) {
		...
		case "useTerminalCommand": {
			const tryQueueCommand = async (retryCount = 0) => {
				const currentTask = provider.getCurrentTask()
				if (currentTask) {
					currentTask.postprocess._execute_command = message.text ?? ""
					return
				}

				if (retryCount < 5) {
					setTimeout(() => {
						void tryQueueCommand(retryCount + 1)
					}, 100)
					return
				}

				console.error("[webviewMessageHandler] Unable to queue terminal command: no active task.")
			}

			void tryQueueCommand()
			break
		}
		case "saveMemory":
		...
		case "createSkill": {
			try {
				...
				if (!source) {
					provider.log("Missing source for createSkill")
					break
				}
				...
				if (source === "global") {
					const globalConfigDir = path.join(os.homedir(), ".roo")
					skillsDir = path.join(globalConfigDir, "skills")
				} else {
					if (!vscode.workspace.workspaceFolders?.length) {
						vscode.window.showErrorMessage(t("common:errors.no_workspace"))
						return
					...
					const workspaceRoot = getCurrentCwd()
					if (!workspaceRoot) {
						vscode.window.showErrorMessage("No workspace folder found for project skill")
						break
					}
					skillsDir = path.join(workspaceRoot, ".roo", "skills")
				}
				...
				if (rawName.trim()) {
					...
					skillName = "new-skill"
					let counter = 1
					let candidateDir = path.join(skillsDir, skillName)
					...
					while (
						...
						skillName = `new-skill-${counter}`
						candidateDir = path.join(skillsDir, skillName)
						counter++
					}
				}
				...
				const templateContent = `---\nname: ${skillName}\ndescription: Describe when to use this skill.\n---\n\n# ${skillName}\n\n## Instructions\n\n- Describe the steps and guidance for this skill.\n`

				await fs.writeFile(filePath, templateContent, "utf8")
				provider.log(`Created new skill file: ${filePath}`)
				...
				vscode.window.showErrorMessage("Failed to create skill")
			}
			break
		}
