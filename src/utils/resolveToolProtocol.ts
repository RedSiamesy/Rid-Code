 ...
 * XML tool protocol is deprecated, but still used as a fallback when a model
 * explicitly does not support native tools. User/profile preferences
 * (`providerSettings.toolProtocol`) and model defaults (`modelInfo.defaultToolProtocol`)
 * are ignored.
 *
 * Precedence:
 * 1. Locked Protocol (task-level lock for resumed tasks - highest priority)
 * 2. XML (when model does not support native tools)
...
export function resolveToolProtocol(
	...
	// 2. Use XML when a model explicitly does not support native tools.
	const supportsNativeTools = (modelInfo as { supportsNativeTools?: boolean } | undefined)?.supportsNativeTools
	if (supportsNativeTools === false) {
		return TOOL_PROTOCOL.XML
	}

	// 3. Default to native for new tasks.
